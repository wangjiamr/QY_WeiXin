...
import la.service.common.SeachEntity;
import la.service.common.SeachRule;
...
public abstract class ActionSupport<T> extends BaseAction {
    ...
    protected String mdURI = "https://api.la.service.com/auth2/authorize";
    ...
    protected String getSTATE_URL(String companyId, String userId, String key, Long value) throws Exception {
        String stateUrl = "";
        AppConfig appConfig = AppConfigUtil.create();
        if (appConfig != null) {
            stateUrl = appConfig.getRedirectUri();
            if (StringUtils.isNotBlank(companyId) && StringUtils.isNotBlank(userId)) {
                stateUrl += "?code=refresh";
                stateUrl += "&type=" + key;
                stateUrl += "&id=" + value;
                stateUrl += "&cId=" + companyId;
                stateUrl += "&uId=" + userId;
            }
        }
        return stateUrl;
