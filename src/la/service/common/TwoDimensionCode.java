...
public class TwoDimensionCode {
    ...
    public int createQRCode(String content, String imgPath,
        ...
        try {
            ...
            qrcodeHandler.setQrcodeErrorCorrect('M');
            qrcodeHandler.setQrcodeEncodeMode('B');
            qrcodeHandler.setQrcodeVersion(8);//1-40 8:154 9:182

            // System.out.println(content);
            byte[] contentBytes = content.getBytes("gb2312");
            // 构造一个BufferedImage对象 设置宽、高
            BufferedImage bufImg = new BufferedImage(200, 200,
                    BufferedImage.TYPE_INT_RGB);
            Graphics2D gs = bufImg.createGraphics();
            ...
            int pixoff = 2;
            // 输出内容 > 二维码
            if (contentBytes.length > 150) {
                System.out.println("内容过程...");
            }
            boolean[][] codeOut = qrcodeHandler.calQrcode(contentBytes);
            for (int i = 0; i < codeOut.length; i++) {
                for (int j = 0; j < codeOut.length; j++) {
            ...
            if (StringUtils.isNotBlank(ccbPath)) {
                Image img = null;
                if (ccbPath.startsWith("http")) {
                ...
                if (img != null) {
                    gs.drawImage(img, 80, 80, 40, 40, null);
                    gs.dispose();
                }
            }
    ...
    public static int createQRCode2(String content, String imgPath,
                                    String ccbPath) {
        try {
            Qrcode qrcodeHandler = new Qrcode();
            qrcodeHandler.setQrcodeErrorCorrect('M');
    ...
    public static void main(String[] args) {
        String imgPath = "/project/java/LA/web/upload/QRCode.png";
        ...
        handler.createQRCode(encoderContent, "/project/java/LA/web/upload/QRCodebg.png",
                "/project/123.png");
        System.out.println("----------------------");
    }
}