define(function(require, exports, module) {
	var $laCommon = require('la_common');
	var $windowManager = require('manager/window');
	var $userInfo = require('core/userInfo');
	var $laIscroll = require('core/la_iscroll');
	$(document).ready(function() {
		$laIscroll.init();
		$laIscroll.use('wrapper', false, null, null);
		$laCommon.touchSE($('#nextStepAction'), function(event, startTouch, o) {
		}, function(event, o) {
			$windowManager.create('step2.html',false,true);
		});
		$laCommon.touchSE($('#quitAction'), function(event, startTouch, o) {
		}, function(event, o) {
			$windowManager.back();
		});
		$laCommon.touchSE($('#uploadAction'), function(event, startTouch, o) {
		}, function(event, o) {

			plus.gallery.pick(function(path) {
				//showImg(path);
			}, function(e) {
				
			});

		});

	});
});

