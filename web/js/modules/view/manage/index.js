define(function(require, exports, module) {
	...
	loadIngData = function() {
		$list.url($laCommon.getRestApiURL() + "/wf/reqManage/ingList");
		$list.params({
		...
		$list.bind(function(jsonData) {
			...
			if (reqList) {
				...
				$(reqList).each(function(i, o) {
					var listTemp = $templete.getManageListTemp();
	...
	$laCommon.onReady(function() {
		...
		$laIscroll.up(function() {
			...
			if ($more.hasMore()) {
				...
				$list.appendBind(function(jsonData) {
					...
					if (reqList) {
						$(reqList).each(function(i, o) {
							var listTemp = $templete.getManageListTemp(true, true);
			...
			window.setTimeout(function() {
				$menu.toggle(function() {
					...
					if (selectMenu == 'ing') {
						...
						$list.url($laCommon.getRestApiURL() + "/wf/reqManage/historyList");
					}
					params = $.extend({}, params, {
						'start' : 0
					});
		...
		$sidebar.menuAction(function(selectMenu) {
			$sidebar.toggle(function() {
				...
				} else if (selectMenu == 'task') {
					$windowManager.load('../task/index.html');
				}
			});
		});
	});
